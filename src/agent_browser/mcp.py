...
class MCPDriver:
    ...
    async def _route_interceptor(self, route: Any):
    ...
    def _handle_console(self, msg: Any):
        if msg.type == "error":
            self.console_errors += 1

    def _handle_pageerror(self, exc: Any):
        self.console_errors += 1
        logging.error(f"Page error: {exc}")

    async def ensure_healthy(self) -> Page:
        """Check health and respawn if crashed. Returns the active page."""
        if not self.browser or not self.browser.is_connected() or not self.page:
            logging.info("Browser disconnected or page missing, respawning...")
            await self.cleanup()
            await self.start()
...
# Using a dummy instance to avoid type errors, will be replaced in main()
_driver_instance: Optional[MCPDriver] = None

def get_driver() -> MCPDriver:
...
async def browser_navigate(url: str) -> BrowserResponse:
    ...
    async with driver._lock:
        try:
            ...
            page = await driver.ensure_healthy()
            await page.goto(valid_url, wait_until="domcontentloaded", timeout=30000)
            return BrowserResponse(success=True, message=f"Navigated to {page.url}")
        except ValueError as e:
...
async def browser_screenshot(name: Optional[str] = None, include_b64: bool = False) -> BrowserResponse:
    """Take a screenshot. Returns path and optionally base64 data."""
    driver = get_driver()
    async with driver._lock:
        try:
            page = await driver.ensure_healthy()
            ...
            # Note: WebP might not be supported in all environments, using PNG for safety
            await page.screenshot(path=str(path))

            data: Dict[str, Any] = {"path": str(path.absolute())}
            if include_b64:
...
async def browser_fill(selector: str, value: str) -> BrowserResponse:
    ...
    async with driver._lock:
        try:
            ...
            return BrowserResponse(success=True, message=f"Filled {selector}")
        except Exception as e:
            return BrowserResponse(success=False, message=str(e), error_code=BrowserErrorCode.EXECUTION_ERROR)
    ...
    async with driver._lock:
        try:
            ...
            if selector:
                ...
                return BrowserResponse(success=True, message=f"Selector {selector} is {state}")
            else:
                await page.wait_for_load_state("networkidle", timeout=timeout)
                return BrowserResponse(success=True, message="Reached networkidle")
        except Exception as e:
            return BrowserResponse(success=False, message=str(e), error_code=BrowserErrorCode.EXECUTION_ERROR)
...
async def browser_handle_dialog(action: Literal["accept", "dismiss"], prompt_text: Optional[str] = None) -> BrowserResponse:
    ...
    return BrowserResponse(success=False, message="Not implemented", error_code=BrowserErrorCode.EXECUTION_ERROR)

@mcp.tool()
async def browser_query(selector: str) -> BrowserResponse:
    """Query elements for a simplified view."""
    driver = get_driver()
    async with driver._lock:
        try:
            page = await driver.ensure_healthy()
            ...
            (selector) => {
                ...
                const mapEl = (node) => {
                    ...
                    return {
                        ...
                        children: Array.from(node.children).map(mapEl).filter(n => n !== null).slice(0, 5)
                    };
                };
                return mapEl(el);
            }
            """
            result = await page.evaluate(js_query, selector)
            if not result:
                return BrowserResponse(success=False, message="Not found", error_code=BrowserErrorCode.SELECTOR_NOT_FOUND)
            return BrowserResponse(success=True, message="Query successful", data=cast(Dict[str, Any], result))
        except Exception as e:
            return BrowserResponse(success=False, message=str(e), error_code=BrowserErrorCode.EXECUTION_ERROR)
...
async def browser_get_logs() -> BrowserResponse:
    ...
    driver = get_driver()
    async with driver._lock:
        return BrowserResponse(success=True, message="Logs retrieved", data={"console_errors": driver.console_errors})
...
async def browser_status() -> BrowserStatus:
    ...
    driver = get_driver()
    if not driver.page:
        return BrowserStatus(url="none", title="none", load_state="none", is_healthy=False, console_errors=0)
...
def main():
    ...
    _driver_instance = MCPDriver(headless=not args.visible, allow_private=args.allow_private)

    loop = asyncio.get_event_loop()
    loop.run_until_complete(_driver_instance.start())
